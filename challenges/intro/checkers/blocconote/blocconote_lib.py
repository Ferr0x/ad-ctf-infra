...
class CheckMachine:
    ...
    def put_note(self, note_name: str, note_value: str):
    ...
    def get_note(self, note_name: str, status: Status) -> str:
        ...
        self.c.assert_in("note", data, "Invalid response on get_note", status)
        self.c.assert_eq(type(data["note"]), str, "Invalid response on get_note", status)
